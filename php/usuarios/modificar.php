<?php
    ...
    try {
        ...
        $sql = "SELECT id FROM admins WHERE LOWER(usuario) = ? AND estado <> ? AND id <> ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssi", $usuarioMin, $estadoVerify, $id);
        $stmt->execute();
        $stmt->store_result();
        ...
        if(!$stmt->execute()) throw new Exception("Ha ocurrido un problema al guardar los cambios, por favor inténtelo nuevamente.");
